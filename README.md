...
- Один постоянный чат отчетов на действующего клиента: все отчеты и вопросы по ним сохраняются в одном месте.
...
Fallback mode: `USE_LLAMA=false`. Ответ строится локально по правилам, категории, эмоциональному тону, уточняющим вопросам и подготовленным комментариям. Выключенные категории не используются для новых ответов: при попадании классификатора в выключенную категорию система переходит к активной категории `Другое`, если она доступна. Выключенные подготовленные комментарии и комментарии из выключенных категорий не попадают в генерацию.
